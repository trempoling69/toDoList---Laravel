            ...
            <div class="w-full rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 rounded-xl bg-white/30 dark:bg-white/10 shadow-lg ring-1 ring-black/5">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method='POST'>
                        ...
                        <button type="submit" class="w-full text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">S'enregistrer</button>
                        <p class="text-sm font-light text-gray-800 dark:text-gray-400">
                            Vous avez déjà un compte ? <a href="{{ route('login') }}" class="font-medium text-black hover:underline dark:text-primary-500 dark:text-white">Se connecter</a>
                        </p>
                    </form>
                    @if ($errors->any())
